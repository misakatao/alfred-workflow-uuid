  ...
  class UUID
    ...
    def initialize(time_low, time_mid, time_hi_and_version,
      ...
      unless time_low >= 0 && time_low < 4294967296
        raise ArgumentError, "Expected unsigned 32-bit number for time_low, got #{time_low}."
      end
      unless time_mid >= 0 && time_mid < 65536
        raise ArgumentError, "Expected unsigned 16-bit number for time_mid, got #{time_mid}."
      end
      unless time_hi_and_version >= 0 && time_hi_and_version < 65536
        raise ArgumentError, "Expected unsigned 16-bit number for time_hi_and_version, got #{time_hi_and_version}."
      end
      unless clock_seq_hi_and_reserved >= 0 && clock_seq_hi_and_reserved < 256
        raise ArgumentError, "Expected unsigned 8-bit number for clock_seq_hi_and_reserved, got #{clock_seq_hi_and_reserved}."
      end
      unless clock_seq_low >= 0 && clock_seq_low < 256
        raise ArgumentError, "Expected unsigned 8-bit number for clock_seq_low, got #{clock_seq_low}."
      end
      unless nodes.kind_of?(Enumerable)
        raise TypeError, "Expected Enumerable, got #{nodes.class.name}."
      end
      unless nodes.size == 6
        raise ArgumentError, "Expected nodes to have size of 6."
      end
      for node in nodes
        unless node >= 0 && node < 256
          raise ArgumentError, "Expected unsigned 8-bit number for each node, got #{node}."
        end
      end
      @time_low = time_low
      @time_mid = time_mid
    ...
    def self.parse(uuid_string)
      unless uuid_string.kind_of? String
        raise TypeError, "Expected String, got #{uuid_string.class.name} instead."
      end
      ...
      return self.new(time_low, time_mid, time_hi_and_version, clock_seq_hi_and_reserved, clock_seq_low, nodes)
    end
    ...
    def self.parse_raw(raw_string)
      unless raw_string.kind_of? String
        raise TypeError, "Expected String, got #{raw_string.class.name} instead."
      end
      ...
      time_low = ((raw_bytes[0] << 24) + (raw_bytes[1] << 16) + (raw_bytes[2] << 8) + raw_bytes[3])
      time_mid = ((raw_bytes[4] << 8) + raw_bytes[5])
      time_hi_and_version = ((raw_bytes[6] << 8) + raw_bytes[7])
    ...
    def self.parse_int(uuid_int)
      unless uuid_int.kind_of?(Integer)
        raise ArgumentError, "Expected Integer, got #{uuid_int.class.name} instead."
      end
    ...
    def self.parse_hexdigest(uuid_hex)
      unless uuid_hex.kind_of?(String)
        raise ArgumentError, "Expected String, got #{uuid_hex.class.name} instead."
      end
    ...
    def self.timestamp_create(timestamp = nil)
      ...
      @@mutex.synchronize do
        ...
        gmt_timestamp_100_nanoseconds = (gmt_timestamp.tv_sec * 10000000) + (gmt_timestamp.tv_usec * 10) + 0x01B21DD213814000
        ...
        if clock_sequence.nil?
          clock_sequence = self.convert_byte_string_to_int(SecureRandom.random_bytes(16))
        end
    ...
    def valid?
      if [0b000, 0b100, 0b110, 0b111].include?(self.variant) && (1..5).include?(self.version)
        return true
      else
        return false
      end
    ...
    def mac_address
      ...
      return (self.nodes.collect { |node| sprintf("%2.2x", node) }).join(":")
    end
    ...
    def timestamp
      ...
      gmt_timestamp_100_nanoseconds += ((self.time_hi_and_version & 0x0FFF) << 48)
      gmt_timestamp_100_nanoseconds += (self.time_mid << 32)
      gmt_timestamp_100_nanoseconds += self.time_low
      return Time.at((gmt_timestamp_100_nanoseconds - 0x01B21DD213814000) / 10000000.0)
    end
    ...
    def hexdigest
      (self.frozen? ? generate_hexdigest : (@hexdigest ||= generate_hexdigest)).dup
    end
    ...
    def generate_i
      return (
        begin
          bytes = (time_low << 96) + (time_mid << 80) + (time_hi_and_version << 64) + (clock_seq_hi_and_reserved << 56) + (clock_seq_low << 48)
          6.times do |i|
    ...
    # @api private
    def generate_s
      result = sprintf("%8.8x-%4.4x-%4.4x-%2.2x%2.2x-", @time_low, @time_mid, @time_hi_and_version, @clock_seq_hi_and_reserved, @clock_seq_low);
      6.times do |i|
    ...
    def self.create_from_hash(hash_class, namespace, name)
      ...
      new_uuid = self.parse("#{hash_string[0..7]}-#{hash_string[8..11]}-#{hash_string[12..15]}-#{hash_string[16..19]}-#{hash_string[20..31]}")
